...
dataDict = getDataDict(testProteinList,ACTIVE_PATH,DECOY_PATH,contactPath) # this actually the testDataSet
...
# modelArgs['attention_type'] = 'multi-head dot-product'
# modelArgs['attention_type'] = 'multi-head additive'
modelArgs['attention_type'] = 'transformer-encoder'
print(json.dumps(modelArgs, indent=4))

print('train args...')

trainArgs = {}
trainArgs['model'] = DrugVQA(modelArgs,block = ResidualBlock).cuda()
# trainArgs['model'] = DrugVQA(modelArgs,block = ResidualBlock)
# trainArgs['epochs'] = 10
trainArgs['epochs'] = 5
trainArgs['lr'] = 0.0007
...
# trainArgs['criterion'] = LabelSmoothingLoss()
# trainArgs['criterion'] = FocalLoss()

# Weight decay > 0 => L2 regularization
# https://stackoverflow.com/questions/42704283/l1-l2-regularization-in-pytorch
trainArgs['optimizer'] = torch.optim.Adam(trainArgs['model'].parameters(),lr=trainArgs['lr'], weight_decay=0) #1e-5)
trainArgs['doSave'] = True
trainArgs['saveNamePre'] = 'martinDUDE30Res-fold3-'
print(json.dumps({k: str(val)[:20] for k, val in trainArgs.items()}, indent=4))
